...
int port_sum_count = 0;  // Condition 3 counter
...
vector<int> packet_sizes;
 ...
 ofstream packet_size_file("packet_size.txt");
...
void analyze_packet_conditions(const struct ip *ip_header, const struct tcphdr *tcp_header) {
    ...
    data_transferred[flow_pair] += ntohs(tcp_header->th_off);  // Add packet size to the flow's total
}
...
void process_packet(const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    packet_size_file << packet_size << endl;
...
void display_results() {
    ...
    output_file << "  - Total Packets: " << total_packets << endl;
    ...
    string command = "python3 histogram.py";
    ...
    for (const auto &flow : data_transferred) {
    ...
    double capture_duration = end_time - start_time;
    if (capture_duration > 0) {
        double pps = total_packets / capture_duration;
        double mbps = (total_data * 8.0) / (capture_duration * 1e6);
        printf("\n[Capture Speed]\n");
        output_file << "\n[Capture Speed]\n";
...
void display_matching_conditions() {
    ...
    output_file << "\n[Condition 3: Count of Packets with Port Sum between 10,000 and 20,000]\n";
    printf("  - Total Packets: %d\n", port_sum_count);
    output_file << "  - Total Packets: " << port_sum_count << endl;
...
void capture_packets(const char *interface) {
    ...
    pcap_close(handle);
    display_results();
    display_matching_conditions();
}

void cleanup() {
    output_file.close();
    packet_size_file.close();
}
